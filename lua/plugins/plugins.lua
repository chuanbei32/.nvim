...
-- global {
    ...
    inserts('tpope/vim-repeat', 'vim-repeat') -- 映射重复
-- }
...
-- window {
    ...
    inserts('https://gitlab.com/yorickpeterse/nvim-window', 'nvim-window') -- 窗口切换
    inserts('simnalamburt/vim-mundo') -- 撤销
    inserts('gennaro-tedesco/nvim-peekup') -- 寄存器/宏
    inserts('skywind3000/asyncrun.vim', 'asyncrun') -- 异步运行
    inserts('skywind3000/asynctasks.vim', 'asynctasks') -- 异步任务
    -- 文件管理器 {
...
-- buffer {
    ...
    inserts({'nvim-treesitter/nvim-treesitter'}, 'nvim-treesitter') -- 突出显示
    inserts('norcalli/nvim-colorizer.lua', 'nvim-colorizer.lua') -- 编码颜色转视觉颜色
    inserts('folke/twilight.nvim', 'twilight') -- 专注模式
    inserts('phaazon/hop.nvim') -- 跳转高亮
    inserts('hrsh7th/vim-eft', 'vim-eft') -- f/t 搜索/查找高亮
    -- inserts('kevinhwang91/nvim-hlslens') -- / 搜索/查找高亮
    inserts('RRethy/vim-illuminate', 'vim-illuminate') -- 高亮光标下词
    inserts({
